...
const handleLogin = () => {
    if (adminKeyInput.value === SECRET_ADMIN_KEY) {
        ...
        authError.value = '';
        loadAdminData(); // Загружаем данные только после ввода ключа
    } else {
...
const totalMessages = ref(0); // Счетчик всех сообщений
...
const loadAdminData = async () => {
    // Пока здесь тестовые данные (в одном формате, как ты просил)
    totalRegistered.value = 142;
    onlineCount.value = 12;
    totalMessages.value = 15843; // Имитация количества сообщений
...
const handleBanUser = (user: User) => {
    ...
    // Локальное переключение статуса бана
...
.table-container {
    ...
    border: 1px solid var(--border);
    border-radius: 12px;
    overflow-x: auto; /* Адаптивность для таблицы */
}
...
@media (max-width: 600px) {
    ...
    .hide-mobile { display: none; } /* Скрываем текст "Назад", оставляем только стрелочку */
